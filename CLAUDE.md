...
- 조사는 `josa(name,'이','가')` 로 처리 (받침 판별). 부르는 말도 `josa(name,'아','야')`
- 사람 이름을 문장에 넣을 때는 `fullNameOf(member)` — `name`/`full_name` 이 겹쳐도 한 번만 나옴.
  한글은 조합형(NFD)·완성형(NFC) 이 섞여 들어올 수 있어 비교 전에 `normalize('NFC')` 필수
