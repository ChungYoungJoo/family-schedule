...
- 아이 아이콘은 `members.emoji` 에 `cat:cheese` / `cat:grey` 를 넣고 `avatarOf(m)` 이
  1em 짜리 SVG 고양이를 그림. 이모지로는 털색을 구분할 수 없어서 도입.
  **시트 제목처럼 `textContent` 로 들어가는 자리에는 `emojiOf(m)`** 를 써야 함
